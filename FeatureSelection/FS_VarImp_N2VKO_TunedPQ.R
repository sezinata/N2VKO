...
for(dis in 1:7){
  ...
  for(z in 1:2){
    ...
    for (j in 1:k){
      ...
      model <- glm(Labels~., data = DATA, family = "gaussian")
      importance[[j]]<- varImp(model, scale=TRUE) #NORMALIZE BETWEEN O AND 100 SCALE.
      
      resultdf=cbind(rownames(importance[[j]]),importance[[j]])
      resultdf<-resultdf[order(-resultdf$Overall),]
      topvector<-seq.int(10, 500, 10) #from , to, by =10
      for(to in 1:length(topvector)){
        path<- paste0(pathWrite,"ImportanceVarImpTOP",topvector[to],"/")
        dir.create(path,recursive = TRUE,showWarnings = FALSE)
        
        write.table(resultdf[1:topvector[to],], file = paste0(path,disease,dataset,j,"KeywordsEmbeddingsUsefulSubset.txt"),append = FALSE,row.names = TRUE,col.names = FALSE,quote = FALSE, sep = "\t")
      }
    }
  }
}
