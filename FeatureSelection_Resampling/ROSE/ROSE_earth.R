...
features<-c("earthVarimpImportance")
featuresFolders<-c("earthVarimpImportance")
  ...
  for(feat in 1:length(features)){
    ...
    for(z in 1:2){
      ...
      for(para in 1:9){
        ...
        for (j in 1:k){
          ...
          if(length(selectedkeywordsnoNAs)>0){
            ...
            if(ncol(xtrain)==1){
              ...
              sum<-sum+auc(roc(glmpredict,label_test))#BestOversampling  "ROSE0x" 
        ...
        write.table(DATA, file = paste0(pathWrite,"earthROSEParameterScores.txt"),quote=FALSE,append = TRUE,col.names = FALSE, row.names = FALSE, sep = " ")
