...
for(to in seq(10, 500, by = 10) ){
  features[ifeat]<- paste0("ImportanceVarImpTOP",to)
  featuresFolders[ifeat]<-paste0("ImportanceVarImpTOP",to)
  ifeat<-ifeat+1
}
  ...
  for(feat in 1:length(features)){
    ...
    for(z in 1:2){
      ...
      for(para in 1:9){
        ...
        write.table(DATA, file = paste0(pathWrite,"VarImpSMOTEParameterScores.txt"),quote=FALSE,append = TRUE,col.names = FALSE, row.names = FALSE, sep = " ")
