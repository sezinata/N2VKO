...
features<-c("StepwiseLM")
featuresFolders<-c("StepwiseLM")
  ...
  for(feat in 1:length(features)){
    ...
    for(z in 1:2){
      ...
      for(para in 1:9){
        ...
        write.table(DATA, file = paste0(pathWrite,"StepwiseLMSMOTEParameterScores.txt"),quote=FALSE,append = TRUE,col.names = FALSE, row.names = FALSE, sep = " ")
